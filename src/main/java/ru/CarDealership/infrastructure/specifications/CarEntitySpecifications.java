...
public class CarEntitySpecifications {
    ...
    public static Specification<CarEntity> hasPowerBetween(Integer minPower, Integer maxPower) {
        return (root, query, cb) -> {
            ...
            if (minPower != null) {
                predicate = cb.and(predicate, cb.greaterThanOrEqualTo(model.get("enginePower"), minPower));
            }
            if (maxPower != null) {
                predicate = cb.and(predicate, cb.lessThanOrEqualTo(model.get("enginePower"), maxPower));
            }
            return predicate;
        };
    }
    ...
    public static Specification<CarEntity> hasEngineVolumeBetween(Double minVolume, Double maxVolume) {
        return (root, query, cb) -> {
            ...
            Predicate predicate = cb.conjunction();
            if (minVolume != null) {
                predicate = cb.and(predicate, cb.greaterThanOrEqualTo(model.get("engineVolume"), minVolume));
            }
            if (maxVolume != null) {
                predicate = cb.and(predicate, cb.lessThanOrEqualTo(model.get("engineVolume"), maxVolume));
            }
            return predicate;
        };
    }
    ...
    public static Specification<CarEntity> hasPriceBetween(BigDecimal minPrice, BigDecimal maxPrice) {
        return (root, query, cb) -> {
            ...
            Predicate predicate = cb.conjunction();
            if (minPrice != null) {
                predicate = cb.and(predicate, cb.greaterThanOrEqualTo(model.get("price"), minPrice));
            }
            if (maxPrice != null) {
                predicate = cb.and(predicate, cb.lessThanOrEqualTo(model.get("price"), maxPrice));
            }
            return predicate;
        };
    }
    ...
    public static Specification<CarEntity> hasColor(String color) {
        return (root, query, cb) -> {
            ...
            Predicate colorNotNull = cb.isNotNull(parts.get("color"));
            Predicate colorMatch = cb.equal(cb.lower(parts.get("color")), color.toLowerCase());
            
            if (query != null) {
                query.distinct(true);
            }
            
            return cb.and(colorNotNull, colorMatch);
        };
    }
    ...
    public static Specification<CarEntity> hasTransmissionType(TransmissionType transmissionType) {
        return (root, query, cb) -> {
            ...
            Predicate transmissionNotNull = cb.isNotNull(parts.get("transmissionType"));
            Predicate transmissionMatch = cb.equal(parts.get("transmissionType"), transmissionType);
            
            if (query != null) {
                query.distinct(true);
            }
            
            return cb.and(transmissionNotNull, transmissionMatch);
        };
    }
