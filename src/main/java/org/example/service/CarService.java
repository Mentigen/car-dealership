...
public class CarService {
    ...
    public List<Part> getAvailablePartsForModel(UUID modelId) {
        ...
        return partRepository.findAll().stream()
                .filter(part -> part.isCompatibleWith(model))
                .toList();
    }
    ...
    public List<Car> searchCars(CarFilter filter) {
        ...
        .filter(car -> {
            ...
            if (filter.getBodyType() != null
                && model.getBody() != filter.getBodyType()) return false;
            if (filter.getFuelType() != null
                && model.getFuel() != filter.getFuelType()) return false;
            if (filter.getDriveType() != null
                && model.getDrive() != filter.getDriveType()) return false;

            if (filter.getMinPower() != null
                && model.getEnginePower() < filter.getMinPower()) return false;
            if (filter.getMaxPower() != null
                && model.getEnginePower() > filter.getMaxPower()) return false;
            ...
            if (filter.getColor() != null) {
                boolean hasColor = car.getCarConfiguration().getParts().stream()
                    .filter(part -> part instanceof ColorPart)
                    .map(part -> (ColorPart) part)
                    .anyMatch(colorPart -> colorPart.getColor().equalsIgnoreCase(filter.getColor()));

                if (!hasColor) return false;
            }

            if (filter.getTransmissionType() != null) {
                boolean hasTransmission = car.getCarConfiguration().getParts().stream()
