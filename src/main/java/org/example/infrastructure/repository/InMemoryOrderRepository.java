...
public class InMemoryOrderRepository implements OrderRepository {
    ...
    public List<Order> findByCustomer(User customer) {
        return storage.values().stream()
                .filter(order -> order.getClient().equals(customer)).toList();
    }

    @Override
    public List<Order> findByManager(User manager) {
        return storage.values().stream()
                .filter(order -> order.getManager().equals(manager)).toList();
    }

    @Override
    public List<Order> findByStatus(String status) {
        return storage.values().stream()
                .filter(order -> order.getState().getName(order).equalsIgnoreCase(status)).toList();
    }
