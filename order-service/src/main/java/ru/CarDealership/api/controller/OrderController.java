...
public class OrderController {
    ...
    public StockOrderResponse createStockOrder(
        ...
        var carInfo = storageClient.getCarById(request.getCarId());
        return mapper.toStockResponse(orderService.createStockOrder(client, request.getCarId(), carInfo.price()));
    }

    @PostMapping("/custom")
    @ResponseStatus(HttpStatus.CREATED)
    @Operation(summary = "Create custom order")
    @PreAuthorize("hasAnyRole('USER', 'ADMIN')")
    public CustomOrderResponse createCustomOrder(
            @Valid @RequestBody CreateCustomOrderRequest request,
            Authentication authentication) {
        User client = userResolver.resolve(authentication);
        var cfgInfo = storageClient.getCarConfigurationById(request.getCarConfigurationId());
        return mapper.toCustomResponse(orderService.createCustomOrder(client, request.getCarConfigurationId(), cfgInfo.price()));
    }
