...
public interface PartDtoMapper {
    ...
    default PartResponse toPolymorphicResponse(Part part) {
        if (part instanceof ColorPart colorPart) return toColorResponse(colorPart);
        if (part instanceof TransmissionPart transmissionPart) return toTransmissionResponse(transmissionPart);
