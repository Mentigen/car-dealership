...
public class PartController {
    ...
    public List<PartResponse> findAll() {
        return partService.getAllParts().stream().map(mapper::toPolymorphicResponse).toList();
    }
    ...
    public List<PartResponse> findCompatibleWithModel(@PathVariable UUID modelId) {
        return carService.getAvailablePartsForModel(modelId).stream().map(mapper::toPolymorphicResponse).toList();
    }
